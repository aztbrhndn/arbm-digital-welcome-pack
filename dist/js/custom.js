...
Reveal.initialize({
    ...
    menu: {
        ...
        themes: [
            ...
            { name: 'Solarized', theme: 'lib/reveal.js/dist/theme/solarized.css' }
        ],
...
Reveal.addEventListener('slidechanged', function(event) {
    ...
    document.documentElement.style.setProperty('--slide-progress', `${progress * 100}%`);
    
    // Update page title
    const slideTitle = event.currentSlide.getAttribute('data-menu-title') || 'ARBM Digital Welcome Pack';
    document.title = `${slideTitle} - ARBM Digital Welcome Pack`;
});

// Matterport tour loading function
function loadMatterportTour() {
    const matterportPlaceholder = document.querySelector('.matterport-placeholder');
...
Reveal.addEventListener('slidechanged', function(event) {
    localStorage.setItem('arbm-presentation-state', JSON.stringify({
        slide: event.indexh,
        timestamp: new Date().toISOString()
    }));
});
